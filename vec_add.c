Datum vec_add_with_vec(PG_FUNCTION_ARGS);
PG_FUNCTION_INFO_V1(vec_add_with_vec);

/**
 * Does element-wise addition between two vectors.
 *
 ...
 */
Datum
vec_add_with_vec(PG_FUNCTION_ARGS)
{
  Oid elemTypeId;
  int16 elemTypeWidth;
  ...
  if (elemTypeId != ARR_ELEMTYPE(rhsArray)) {
    ereport(ERROR, (errmsg("vec_add input arrays must be the same type")));
  }
...
Datum vec_add_with_scalar(PG_FUNCTION_ARGS);
PG_FUNCTION_INFO_V1(vec_add_with_scalar);

/**
 * Adds a scalar to all elements of a given vector.
 *
 ...
 */
Datum
vec_add_with_scalar(PG_FUNCTION_ARGS)
{
  ...
  if (elemTypeId1 == INT2OID ||
    ...
    ereport(ERROR, (errmsg("vec_add scalar operand must be a numeric type")));
  }
  ...
  if (ARR_ELEMTYPE(inputArray) != scalarTypeId) {
    ereport(ERROR, (errmsg("vec_add array elements and scalar operand must be the same type")));
  }
